...
class TopicNetworkManager {
    
    static let shared = TopicNetworkManager()
    
    private init() { }
    
    func getTopicPhotoData(topic: String, completionHandler: @escaping ([Photo]) -> Void) {
        let url = "https://api.unsplash.com/topics/\(topic)/photos?page=1&client_id=\(SearchPhotoAPI.clientID)"
